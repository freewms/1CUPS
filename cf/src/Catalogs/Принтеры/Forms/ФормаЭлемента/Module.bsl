...
Процедура ОбновитьСостояниеПринтера()
	...
	Иначе
		СостояниеПринтера = ПредставлениеКодаСостоянияПринтера(Результат[ИмяПринтера]["printer-state"]);
		ОписаниеСостояния = Строка(Результат[ИмяПринтера]["printer-state-message"]);
	КонецЕсли;
