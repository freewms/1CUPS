...
//  Структура - см.НовыйЗапросаСервисаПечати
...
// 
// Возвращаемое значение:
//  Соответствие - Описание результатов
...
Функция ПолучитьСостояниеПринтера(СерверПечати, ИмяПринтера = Неопределено) Экспорт
	...
	МассивИменПринтеров.Добавить(ИмяПринтера);
	ПараметрыМетода.Вставить("printers", МассивИменПринтеров);
	КонецЕсли;
...
// 
// Параметры:
//  Принтеры - Массив - см СправочникСсылка.Принтеры
// 
// Возвращаемое значение:
...
// Возвращаемое значение:
//  Соответствие - Описание результатов
//	 * Принтер - СправочникСсылка.Принтеры
//	 * Соответствие - Описание результата
Функция ОбработатьЗапрос(Метод, ПараметрыМетода)
	...
	ИначеЕсли Метод = Перечисления.МетодыСервераПечати.ПолучитьСостояниеОчереди Тогда
		ПараметрыМетодаЗапроса.Вставить("command", "queues_info");
	ИначеЕсли Метод = Перечисления.МетодыСервераПечати.ОчиститьОчередьПечати Тогда
		ПараметрыМетодаЗапроса.Вставить("command", "clear_queues");
	...
	Для Каждого Запрос Из ЗапросыПоСерверамПечати Цикл
		...
		Для Каждого ЭлементРезультата Из ОписаниеРезультата Цикл
			Результат.Вставить(ЭлементРезультата["id"], ЭлементРезультата);	
		КонецЦикла;
...
//  Метод - ПеречислениеСсылка.МетодыСервераПечати
//  ПараметрыМетода - Соответствие - набор параметров для вызова метода
// 
// Возвращаемое значение:
// 	Соответствие из КлючИЗначение:
//	 * Ключ - Строка
...
//  
// Возвращаемое значение:
//  Соответствие - Задача для печати в нотации, пригодной к сериализации JSON
Функция ПодготовитьДокументКОтправкеНаСерверПечати(Документ, ПараметрыПринтера)
	
	ПотокВПамяти = Новый ПотокВПамяти();
				
	Если Документ.Автомасштаб Тогда
		ПараметрОриентации = ПараметрыПринтера.ОриентацияСтраницы;
		...
		Документ.ОриентацияСтраницы = ?(ПараметрОриентации = Перечисления.ОриентацияСтраницы.Альбомная, 
